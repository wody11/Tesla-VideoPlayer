import { adtsToRaw, getAdtsInfo, extractASC } from './aac-adts-raw-ca5ce116.js';

// 辅助：从 ES 数据中提�?NALU（AnnexB 格式，支�?3/4 字节起始码）
...
function demuxTS(buffer) {
    ...
    function flushPes(pid) {
        ...
        if (pid === vPid) {
            ...
            // �?AUD(NAL type=9) �?slice(首片) 分帧，确保每�?EncodedVideoChunk 对应一�?AU
            const groups = [];
            let cur = [];
            ...
            if (cur.length)
                groups.push(cur);
            // 仅保留包含至少一�?slice �?AU，避免仅�?SEI/AUD 的无效帧
            ...
            for (const g of auList) {
                ...
                // 重新组装 AnnexB（为每个 NALU �?4 字节起始码）
                // 确保每个 AU 前都�?AUD（NAL type=9），部分实现依赖 AUD 分帧
            ...
            for (let i = 0; i < raws.length; i++) {
                const raw = raws[i];
                // 将采样率/声道�?ASC 附带在样本上（便于主线程配置 WebCodecs AudioDecoder�?
...
export { demuxTS };
//# sourceMappingURL=demux-ts-eef4087b.js.map
