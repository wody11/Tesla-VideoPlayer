...
async function runHlsSession(url, startSeq) {
    ...
    // ABR / fetch tuning
    let masterVariants = null;
    let currentVariantIndex = 0;
    let ewmaFetchMs = null;
    const EWMA_ALPHA = 0.2;
    // 当 VOD seek 刚发生后，尝试对首片做部分 Range 拉取以加速首画
    let justSeekedToSeq = null;
    ...
    let lastAudioCfg = {};
    let lastAvccSig = null;
    let lastParamSps = null;
    let lastParamPps = null;
    let injectParamFrames = 0; // 在参数更新后，前若干个关键帧内带内嵌 SPS/PPS
    ...
    let lastDurationMs = undefined; // HLS VOD 总时长
    let pendingKeyHandlerRunning = false; // 防止并行的 aggressiveKeySearch
    const livePoll = async (delayMs) => new Promise(r => setTimeout(r, delayMs));
    // aggressiveKeySearch will be defined later where demuxTS/keyBytes etc are in scope
    function toHex2(n) { return (n & 0xff).toString(16).padStart(2, '0').toUpperCase(); }
    ...
    async function resolveMedia(u) {
        ...
        if (/EXT-X-STREAM-INF/.test(text)) {
            ...
            if (!master.length)
                throw new Error('HLS master has no variants');
            // record variant list for potential ABR
            try {
                masterVariants = master.map((m) => m.url);
            }
            catch (e) {
                masterVariants = master.map((m) => m.url);
            }
            ...
            try {
                gHlsVariants = masterVariants ? masterVariants.slice() : null;
            ...
            masterUrlStr = u;
            try {
                const list = master.map((m, i) => ({ index: i, url: m.url }));
    ...
    while (!hlsAbort) {
        ...
        if (isEnd && Array.isArray(parsed.segments) && parsed.segments.length) {
            try {
                const totalMs = Math.round(parsed.segments.reduce((s, seg) => s + (Number(seg.duration) || 0), 0) * 1000);
        ...
        async function aggressiveKeySearch() {
            ...
            try {
                ...
                while (pendingKeyRequest && !hlsAbort && attempts < maxAttempts) {
                    attempts++;
                    try {
                        const r2 = await fetch(mediaUrl, { cache: 'no-store' });
                        if (!r2.ok)
                            break;
                        const txt2 = await r2.text();
                        const parsed2 = parseM3U8(txt2, mediaUrl);
                        const seqStart = Math.max(parsed2.mediaSequence, lastMediaSeq + 1);
                        const candidates = (parsed2.segments || []).filter((s) => s.seq >= seqStart).slice(0, 6);
                        if (!candidates || candidates.length === 0) {
                            await livePoll(200);
                            continue;
                        }
                        for (const s of candidates) {
                            if (!pendingKeyRequest || hlsAbort)
                                break;
                            try {
                                const rr = await fetch(s.uri, { cache: 'no-store' });
                                ...
                                const vid2 = samples2.find((x) => x.kind === 'video' && !!x.key);
                                if (vid2) {
                                    postMain({ type: 'key-found', seq: s.seq, ts: vid2.tsUs });
                                    pendingKeyRequest = false;
                                    break;
                                }
                                else {
                                    postMain({ type: 'key-not-found', seq: s.seq });
                                }
                            }
                            catch (e) { /* best-effort */ }
                        }
                ...
                pendingKeyHandlerRunning = false;
            }
        }
            ...
            try {
                ...
                if (isEnd && justSeekedToSeq === seg.seq) {
                    try {
                        ...
                        const r = await fetch(seg.uri, { cache: 'no-store', headers: { 'Range': `bytes=0-${rangeBytes}` } });
                        if (r.status === 206 || r.ok) {
                            ...
                            try {
                                ...
                                const vid = samplesPart.find((s) => s.kind === 'video' && !!s.key);
                                if (vid) {
                                    // post partial samples and mark that remainder should be fetched later
                                    for (const s of samplesPart) {
                                        try {
                                            postMain({ type: 'sample', kind: s.kind, ts: s.tsUs, dur: s.durUs, key: s.key, data: s.data }, [s.data]);
                                        }
                                        catch { }
                                    }
                                    ...
                                    (async () => {
                                        try {
                                            const rf = await fetch(seg.uri, { cache: 'no-store' });
                                            ...
                                            for (const s2 of samples2) {
                                                try {
                                                    postMain({ type: 'sample', kind: s2.kind, ts: s2.tsUs, dur: s2.durUs, key: s2.key, data: s2.data }, [s2.data]);
                                                }
                                                catch { }
                                            }
                                        }
                                    ...
                                    try {
                                        postMain({ type: 'hls-pos', seq: lastMediaSeq });
                                    }
                                    catch { }
                ...
                const r = await fetch(seg.uri, { cache: 'no-store' });
                ...
                if (pendingKeyRequest) {
                    try {
                        const vid = samples.find((s) => s.kind === 'video' && !!s.key);
                        if (vid) {
                    ...
                    try {
                        aggressiveKeySearch().catch(() => { });
                    }
                    catch { }
                }
                // 排序：优先按 dtsUs，其次按 tsUs
                samples.sort((a, b) => (a.dtsUs ?? a.tsUs) - (b.dtsUs ?? b.tsUs));
                // 规范化视频时间戳：保证 tsUs/dtsUs 单调递增，并填充每帧 durUs
                try {
                    const vids = samples.filter((x) => x.kind === 'video');
                    if (vids.length > 0) {
                        ...
                        for (let i = 0; i < vids.length; i++) {
                            ...
                            if (i > 0) {
                                const prev = vids[i - 1];
                                if (!(typeof cur.tsUs === 'number'))
                                    cur.tsUs = prev.tsUs + frameDurUs;
                                if (cur.tsUs <= prev.tsUs)
                                    cur.tsUs = prev.tsUs + frameDurUs;
                                // dtsUs 若缺失或回退，同样校正
                                if (typeof cur.dtsUs !== 'number')
                                    cur.dtsUs = cur.tsUs;
                                if (cur.dtsUs <= (prev.dtsUs ?? prev.tsUs))
                                    cur.dtsUs = (prev.dtsUs ?? prev.tsUs) + frameDurUs;
                                // 为前一帧推导 durUs
                        ...
                        if (typeof last.durUs !== 'number' || last.durUs <= 0)
                            last.durUs = frameDurUs;
                    }
                }
                catch { }
                // DISCONTINUITY：重置时间基
                ...
                try {
                    const firstV = samples.find((s) => s.kind === 'video');
                    if (firstV) {
                        ...
                        if (sps.length && pps.length) {
                            ...
                            let sig = '';
                            try {
                                sig = Array.from(avcc).slice(0, 32).join(',') + '|' + avcc.length;
                            }
                            catch { }
                            if (sig !== lastAvccSig) {
                                lastAvccSig = sig;
                ...
                // 音频配置探测（若采样率/声道有变化）
                try {
                    const firstA = samples.find((s) => s.kind === 'audio');
                    if (firstA) {
                        ...
                        if ((!sr || !ch) && firstA.data) {
                            ...
                            if (info) {
                                sr = sr || info.sampleRate;
                                ch = ch || info.channels;
                            }
                        }
                        if (!asc && firstA.data) {
                            try {
                                const a = await extractASC(firstA.data);
                                if (a && a.byteLength)
                                    asc = a;
                            }
                            catch { }
                        }
                        const ascSig = asc ? (() => { try {
                            const u8 = new Uint8Array(asc);
                            return Array.from(u8.slice(0, 8)).join(',') + '|' + u8.length;
                        }
                        catch {
                            return 'n/a';
                        } })() : null;
                        ...
                        if (needUpdate) {
                            ...
                            const audioCfg = { codec: 'mp4a.40.2' };
                            if (typeof sr === 'number')
                                audioCfg.sampleRate = sr;
                            if (typeof ch === 'number')
                                audioCfg.numberOfChannels = ch;
                            if (asc && asc.byteLength)
                                audioCfg.description = asc;
                ...
                for (const s of samples) {
                    ...
                    if (typeof outDts === 'number') {
                        ...
                        if (!isFinite(diff) || Math.abs(diff) > 5000000 || outDts < 0)
                            outDts = outTs;
                    }
                    let data = s.data;
                    ...
                    if (s.kind === 'video') {
                        ...
                        if (s.key && injectParamFrames > 0 && lastParamSps && lastParamPps) {
                            try {
                                const sps = new Uint8Array(lastParamSps);
                                const pps = new Uint8Array(lastParamPps);
                        ...
                        if (debugVidLogged < 6) {
                            try {
                                ...
                                const tsu = Math.max(0, Math.trunc(s.tsUs ?? 0));
                                const dtsu = s.dtsUs !== undefined ? Math.max(0, Math.trunc(s.dtsUs)) : -1;
                        ...
                        if (len <= 0) {
                            ...
                            if (!declaredNoAudio && emptyAudioFrames >= 10) {
                                try {
                                    postMain({ type: 'stream-info', hasAudio: false, hasVideo: true });
                                }
                                catch { }
                    ...
                    const payload = { type: 'sample', kind: s.kind, ts: outTs, dur: durUs, key: s.key, data };
                    if (typeof outDts === 'number')
                        payload.dts = outDts;
                    if (s.pcrUs !== undefined) {
                        try {
                            const pcrRel = s.pcrUs - (basePtsUs || 0);
                            const ok = Number.isFinite(pcrRel) && Math.abs(pcrRel - outTs) <= 5000000 && pcrRel >= -1000000;
                            if (ok)
                                payload.pcr = pcrRel;
                        }
                        catch { }
                    }
                    postMain(payload, [data]);
                    // 打印前几条音频样本调试信息
                    if (s.kind === 'audio' && debugAudLogged < 6) {
                        try {
                            const len = data.byteLength || 0;
                            const tsu = Math.max(0, Math.trunc(s.tsUs ?? 0));
                            const duu = Math.max(0, Math.trunc(s.durUs ?? 0));
                            postMain({ type: 'log', msg: `dbg[a] tsUs=${tsu} durUs=${duu} len=${len} sr=${s.sr || '?'} ch=${s.ch || '?'}` });
                        }
                        catch { }
                        debugAudLogged++;
                    }
                ...
                lastMediaSeq = seg.seq;
                try {
                    postMain({ type: 'log', msg: `posted samples v=${vCount} a=${aCount} seq=${lastMediaSeq}` });
                }
                catch { }
                try {
                    postMain({ type: 'hls-pos', seq: lastMediaSeq });
...
function postMain(msg, transfer) {
    try {
...
function parseEsdsSimple(esdsBuf) {
    if (!esdsBuf)
        return null;
    try {
        const u8 = esdsBuf instanceof Uint8Array ? esdsBuf : new Uint8Array(esdsBuf);
        let i = 0;
        while (i < u8.length) {
            const tag = u8[i++];
            let len = 0;
...
function setupMp4Box(mp4box) {
    let vTrack = null;
    let aTrack = null;
    mp4box.onError = (e) => postMain({ type: 'log', msg: 'mp4box error ' + String(e) });
    mp4box.onReady = (infoBox) => {
        postMain({ type: 'log', msg: 'mp4box ready tracks=' + (infoBox.tracks ? infoBox.tracks.length : 0) });
        vTrack = infoBox.tracks && infoBox.tracks.find((t) => t.type === 'video');
        aTrack = infoBox.tracks && infoBox.tracks.find((t) => t.type === 'audio');
        // compute durationMs if available
        let durationMs = null;
        try {
            if (infoBox && infoBox.duration && infoBox.timescale)
                durationMs = Math.round(infoBox.duration * 1000 / infoBox.timescale);
            else if (vTrack && vTrack.duration && vTrack.timescale)
                durationMs = Math.round(vTrack.duration * 1000 / vTrack.timescale);
            else if (aTrack && aTrack.duration && aTrack.timescale)
                durationMs = Math.round(aTrack.duration * 1000 / aTrack.timescale);
        }
        catch (e) { /* ignore */ }
        // increase nbSamples to avoid too-frequent callbacks
        if (vTrack)
            mp4box.setExtractionOptions(vTrack.id, null, { nbSamples: 20 });
        if (aTrack)
            mp4box.setExtractionOptions(aTrack.id, null, { nbSamples: 50 });
        mp4box.start();
        // helper: try to parse esds to extract AudioSpecificConfig (ASC) and channel/sampleRate
        function parseEsds(esdsBuf) {
            if (!esdsBuf)
                return null;
            try {
        ...
        try {
            if (vTrack) {
                ...
                if (aTrack) {
                    ...
                    if (aTrack.extra && aTrack.extra.esds)
                        audioInfo.esds = aTrack.extra.esds;
            ...
            postMain({ type: 'log', msg: 'ready-mp4 post failed ' + String(e) });
        }
    };
    mp4box.onSamples = async (id, user, samples) => {
        ...
        for (const s of samples) {
            try {
                ...
                postMain({ type: 'log', msg: 'mp4box post sample failed ' + String(e) });
            }
        }
    };
}
        ...
        case 'setHlsVariant': {
            try {
                if (typeof msg.index === 'number')
                    pendingVariantIndex = Math.floor(Math.max(0, msg.index));
                else if (typeof msg.url === 'string')
                    pendingVariantUrl = String(msg.url);
                postMain({ type: 'log', msg: 'setHlsVariant received' });
            }
            catch (e) {
                postMain({ type: 'log', msg: 'setHlsVariant failed ' + String(e) });
            }
            break;
        }
        case 'openHLS':
            ...
            if (typeof msg.startAtMs === 'number') {
                try {
                    pendingHlsSeekMs = Math.max(0, Number(msg.startAtMs) || 0);
                }
                catch { }
            }
        ...
        case 'fetchSegment':
            (async () => {
                try {
                    ...
                    postMain({ type: 'log', msg: 'fetchSegment failed ' + String(e) });
                }
            })();
            break;
        case 'seek':
            (async () => {
                try {
                    ...
                    if (currentMode === 'mp4' && mp4Session && mp4Session.mp4box) {
                        try {
                            mp4Session.mp4box.seek(ms / 1000, true);
                            postMain({ type: 'log', msg: `seek(MP4) to ${(ms / 1000).toFixed(3)}s requested` });
                        }
                        catch (e) {
                            postMain({ type: 'log', msg: 'mp4box seek failed ' + String(e) });
                        }
                        return;
                    }
                    ...
                    postMain({ type: 'log', msg: 'seek handler error ' + String(e) });
                }
            })();
            break;
        case 'requestKey':
            (async () => {
                try {
                    ...
                    postMain({ type: 'log', msg: 'requestKey handler failed ' + String(e) });
                }
            })();
            break;
        case 'open':
                ...
                try {
                    ...
                    if (MP4Box) {
                        try {
                            ...
                            try {
                                Object.defineProperty(ab, 'fileStart', { value: 0, writable: false, enumerable: true });
                            }
                            catch (e) {
                                ab.fileStart = 0;
                            }
                            ...
                            postMain({ type: 'log', msg: 'mp4box parse failed ' + String(e) });
                        }
                        return;
                    }
                    ...
                    try {
                        ...
                        try {
                            ...
                            postMain({ type: 'log', msg: 'mp4 init log failed ' + String(e) });
                        }
                        ...
                        try {
                            ...
                            if (!samples || samples.length === 0) {
                                try {
                                    ...
                                    postMain({ type: 'log', msg: 'scan boxes failed ' + String(e) });
                                }
                            }
                                ...
                                for (let i = 0; i < max; i++) {
                                    ...
                                    try {
                                        ...
                                        if (s.kind === 'video' && info && info.video && info.video.avcC) {
                                            ...
                                            try {
                                                data = helper.avccToAnnexB(s.data, nalSize);
                                            }
                                            catch (e) { /* fallback */ }
                                        }
                                        ...
                                        postMain({ type: 'log', msg: 'post sample failed ' + String(e) });
                                    }
                                }
                                if (samples.length > max)
                                    postMain({ type: 'log', msg: 'only sent ' + max + ' of ' + samples.length + ' samples' });
                            }
                        }
                        catch (e) {
                            postMain({ type: 'log', msg: 'parseFragment failed ' + String(e) });
                        }
                    }
                    catch (e) {
                        postMain({ type: 'log', msg: 'mp4 parse import error ' + String(e) });
                    }
                }
                catch (e) {
                    postMain({ type: 'log', msg: 'open url error ' + String(e) });
                }
            })();
            break;
        case 'openMP4':
            (async () => {
                try {
                    ...
                    if (MP4Box) {
                        try {
                            ...
                            try {
                                ...
                                if (typeof msg.fileStart === 'number')
                                    Object.defineProperty(ab, 'fileStart', { value: msg.fileStart, writable: false, enumerable: true });
                                else
                                    Object.defineProperty(ab, 'fileStart', { value: 0, writable: false, enumerable: true });
                            }
                    ...
                    postMain({ type: 'log', msg: 'openMP4 failed ' + String(e) });
                }
            })();
            break;
        case 'close':
            ...
            try {
                postMain({ type: 'log', msg: 'worker: closing' });
            }
            catch (e) { }
            hlsAbort = true;
            currentMode = null;
