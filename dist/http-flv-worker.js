import { parseMaster, parseM3U8, timeToSeq } from './playlist-d855fa1e.js';
import { demuxTS } from './demux-ts-8c9ae7ff.js';
import { extractSpsPps } from './h264-annexb-avcc-eae1e80f.js';
import { hexToBytes, seqToIv, decryptAes128CbcPkcs7 } from './aes-58fd1ba1.js';
 ...
 */
function demuxMp4(buffer, post, startTime = 0) {
        ...
        try {
            ...
            if (video) {
                ...
                if (boxType && !description)
                    throw new Error(`MP4 ${boxType} decoder configuration is missing.`);
...
async function fetchMp4WithRanges(url) {
    ...
    const downloadWorker = async () => {
        while (true) {
            ...
            if (response.status !== 206)
                throw new Error(`MP4 Range ${offset}-${end} failed: HTTP ${response.status}.`);
...
//# sourceMappingURL=http-flv-worker.js.map
