...
function splitAnnexBNalus(bytes) {
    const nalus = [];
    const startCodeLength = (offset) => {
        if (offset + 3 <= bytes.length && bytes[offset] === 0 && bytes[offset + 1] === 0 && bytes[offset + 2] === 1)
            return 3;
        if (offset + 4 <= bytes.length && bytes[offset] === 0 && bytes[offset + 1] === 0 && bytes[offset + 2] === 0 && bytes[offset + 3] === 1)
            return 4;
        return 0;
    };
    let offset = 0;
    while (offset < bytes.length) {
        while (offset < bytes.length && startCodeLength(offset) === 0)
            offset += 1;
        ...
        while (offset < bytes.length && startCodeLength(offset) === 0)
            offset += 1;
        if (offset > start)
            nalus.push(bytes.subarray(start, offset));
    }
    return nalus;
}
 ...
 */
function createMp4DemuxSession(post, startTime = 0) {
    ...
    const pump = () => {
        ...
        while (credit > 0 && pending.length > 0) {
            ...
            const duration = Math.max(0, Math.round((sample.duration / sample.timescale) * 1000000));
            if (item.kind === 'video') {
                videoSampleCount += 1;
            ...
            credit -= 1;
            try {
                file.releaseUsedSamples(sample.track_id, sample.number + 1);
            }
            catch { }
        }
        maybePostEnd();
    ...
    file.onReady = (info) => {
        try {
            ...
            if (!video && !audio)
                throw new Error('MP4 contains no playable audio or video track.');
            post({
                type: 'media-info',
            ...
            if (video) {
                ...
                if (!supported)
                    throw new Error(`Unsupported MP4 video codec: ${video.codec}`);
            ...
            if (audio) {
                ...
                const entry = file.getTrackById(audio.id)?.mdia?.minf?.stbl?.stsd?.entries?.[0];
                const description = findDescriptorData(entry?.esds?.esd, 0x05);
                if (!description?.byteLength)
                    throw new Error('MP4 AAC AudioSpecificConfig is missing.');
    ...
    return {
        append(buffer, fileStart, last = false) {
            if (stopped)
                return fileStart;
            const input = buffer;
...
function serializeConfigurationBox(box) {
    if (!box)
        return undefined;
    const stream = new DataStream();
    box.write(stream);
...
function findDescriptorData(descriptor, tag) {
    if (!descriptor)
        return undefined;
    if (descriptor.tag === tag && descriptor.data?.byteLength)
...
function isUsableMediaTimestamp(value) {
...
function readU24(u8, offset) {
    return (u8[offset] << 16) | (u8[offset + 1] << 8) | u8[offset + 2];
}
function readS24(u8, offset) {
    const value = readU24(u8, offset);
    return value & 0x800000 ? value - 0x1000000 : value;
}
function avcCodecFromConfig(record) {
    if (record.length >= 4) {
...
class FlvParser {
    ...
    parse() {
        ...
        while (this.buffer.length - offset >= 15) {
            ...
            if (tagType === 9)
                this.parseVideo(payload, timestampMs);
            else if (tagType === 8)
                this.parseAudio(payload, timestampMs);
            offset = nextOffset;
        }
        if (offset > 0)
            this.buffer = this.buffer.slice(offset);
    }
    parseVideo(payload, timestampMs) {
        if (payload.length < 5)
            return;
        videoTagCount += 1;
    ...
    parseAudio(payload, timestampMs) {
        if (payload.length < 2)
            return;
        audioTagCount += 1;
...
async function openHttpFlv(url) {
    aborter?.abort();
    socket?.close();
    socket = null;
    ...
    try {
        ...
        if (!response.body)
            throw new Error('ReadableStream is not available on fetch response.');
...
async function openMp4(url, startTime = 0) {
    aborter?.abort();
    mp4Session?.stop();
    mp4Session = null;
    aborter = new AbortController();
...
async function streamMp4Ranges(url, session, signal) {
    const chunkSize = 2 * 1024 * 1024;
    const first = await fetchMp4Range(url, 0, chunkSize - 1);
    if (!first.ok)
        throw new Error(`HTTP ${first.status} ${first.statusText}`);
    post({ type: 'stream-open' });
    ...
    while (offset < firstRange.total && !signal.aborted) {
        while (session.bufferedSamples() >= 768 && !signal.aborted)
            await sleep(20);
        if (signal.aborted)
            throw abortError();
        ...
        response = undefined;
        if (current.status !== 206)
            throw new Error(`MP4 Range ${offset}-${expectedEnd} failed: HTTP ${current.status}.`);
...
async function appendResponseStream(response, session, signal) {
    if (!response.body) {
...
async function fetchMp4Range(url, start, end) {
    let lastError = '';
...
async function openHls(url, options = {}) {
    aborter?.abort();
    aborter = new AbortController();
    paused = false;
    videoTagCount = 0;
    ...
    try {
        ...
        if (/EXT-X-STREAM-INF/i.test(firstText)) {
            ...
            if (!variants.length)
                throw new Error('HLS master playlist has no playable variants.');
        ...
        while (!aborter?.signal.aborted) {
            ...
            for (const segment of segments) {
                if (aborter?.signal.aborted)
                    break;
                while (paused && !aborter?.signal.aborted)
                    await sleep(30);
                ...
                if (segment.key?.method === 'AES-128') {
                    if (!segment.key.uri)
                        throw new Error('HLS AES-128 key URI is missing.');
                    ...
                    if (!keyData) {
                        ...
                        if (keyData.byteLength !== 16)
                            throw new Error(`Invalid HLS AES-128 key length: ${keyData.byteLength}.`);
                        keyCache.set(segment.key.uri, keyData);
                    }
                    const iv = segment.key.ivHex ? hexToBytes(segment.key.ivHex) : seqToIv(segment.seq);
                    if (iv.byteLength !== 16)
                        throw new Error(`Invalid HLS AES-128 IV length: ${iv.byteLength}.`);
            ...
            if (playlist.endList && !playlist.segments.some(segment => segment.seq > lastSeq))
                break;
...
function postVideoSample(sample) {
...
function postAudioSample(sample) {
    const data = sample.data;
    post({
        type: 'audio-sample',
        timestamp: sample.tsUs,
...
self.addEventListener('message', (event) => {
    ...
    if (message.type === 'open-http-flv')
        openHttpFlv(message.url);
    else if (message.type === 'open-ws-flv')
        openWsFlv(message.url);
    else if (message.type === 'open-hls')
        openHls(message.url, message);
    else if (message.type === 'open-mp4')
        openMp4(message.url, message.startTime);
    else if (message.type === 'pull')
        mp4Session?.pull(message.count);
