...
function parseInit(arrayBuffer) {
    const dv = new DataView(arrayBuffer);
    const len = dv.byteLength;
    let offset = 0;
    function readUint32(off) { return dv.getUint32(off); }
    ...
    function parseMvhd(start, size) {
        try {
            ...
            if (ver === 1) {
                ...
                if (timescale)
                    info.durationMs = Number(duration * 1000n / BigInt(timescale));
            }
            else {
                const timescale = dv.getUint32(start + 12);
                const duration = dv.getUint32(start + 16);
                if (timescale)
                    info.durationMs = Math.round(duration * 1000 / timescale);
            }
        }
        catch (e) { /* ignore */ }
    }
        ...
        if (type === 'avc1' || type === 'mp4v' || type === 'encv') {
            ...
            while (sub + 8 <= end) {
                ...
                if (stype === 'avcC') {
                    ...
                    try {
                        info.video.nalLengthSize = (info.video.avcC[4] & 0x03) + 1;
                    }
                    catch (e) {
                        info.video.nalLengthSize = 4;
                    }
                    break;
                }
                if (ssz <= 0)
...
function parseFragment(arrayBuffer, info) {
    const dv = new DataView(arrayBuffer);
    const len = dv.byteLength;
    let offset = 0;
    const samples = [];
    function readType(off) { return String.fromCharCode(dv.getUint8(off), dv.getUint8(off + 1), dv.getUint8(off + 2), dv.getUint8(off + 3)); }
    function readUint32(off) { return dv.getUint32(off); }
    while (offset + 8 <= len) {
        const size = readUint32(offset);
        const type = readType(offset + 4);
        if (size <= 8) {
            offset += Math.max(8, size);
            continue;
        }
        const end = offset + size;
        if (type === 'moof') {
            ...
            const mdatOff = scan;
            (mdatOff + 8 <= len) ? readUint32(mdatOff) : 0;
            ...
            while (moofPtr + 8 <= end) {
                ...
                if (childType === 'traf') {
                    ...
                    while (tptr + 8 <= trafEnd) {
                        ...
                        if (ttype === 'tfhd') {
                            dv.getUint8(tptr + 8);
                            dv.getUint8(tptr + 9) << 16 | dv.getUint8(tptr + 10) << 8 | dv.getUint8(tptr + 11);
                            // track id at offset 12
                            dv.getUint32(tptr + 12);
                            ...
                            if (tsz >= 28) {
                                ...
                                try {
                                    default_sample_duration = dv.getUint32(tptr + 20);
                                }
                                catch (e) {
                                    default_sample_duration = 0;
                                }
                                try {
                                    default_sample_size = dv.getUint32(tptr + 24);
                                }
                                catch (e) {
                            ...
                            let data_offset = 0;
                            if (flags & 0x000001) {
                                data_offset = dv.getInt32(cursor);
                                cursor += 4;
                            }
                            if (flags & 0x000004) {
                                cursor += 4;
                            ...
                            for (let i = 0; i < sample_count; i++) {
                                ...
                                let sample_size = 0;
                                if (flags & 0x000100) {
                                    sample_duration = dv.getUint32(cursor);
                                    cursor += 4;
                                }
                                else
                                    sample_duration = default_sample_duration;
                                if (flags & 0x000200) {
                                    sample_size = dv.getUint32(cursor);
                                    cursor += 4;
                                }
                                else
...
function parseMoovSamples(arrayBuffer, info) {
    const dv = new DataView(arrayBuffer);
    const len = dv.byteLength;
    let offset = 0;
    function readType(off) { return String.fromCharCode(dv.getUint8(off), dv.getUint8(off + 1), dv.getUint8(off + 2), dv.getUint8(off + 3)); }
    ...
    while (ptr + 8 <= mdiaOff + mdiaSize) {
        ...
        if (t === 'mdhd') {
            ...
            if (version === 1)
                timescale = dv.getUint32(ptr + 12 + 8);
            else
                timescale = dv.getUint32(ptr + 12);
            break;
        }
        ptr += sz;
    }
        ...
        if (t === 'stsd') {
            ...
            while (p2 + 8 <= end2) {
                ...
                if (ty2 === 'avc1' || ty2 === 'avc3') {
                    ...
                    while (s3 + 8 <= p2 + s2) {
                        ...
                        if (stype === 'avcC') {
                            avcC = new Uint8Array(arrayBuffer.slice(s3 + 8, s3 + ssz));
                            try {
            ...
            if (sample_size === 0) {
                ...
                for (let i = 0; i < count; i++) {
                    sampleSizes.push(dv.getUint32(p));
                    p += 4;
                }
            }
            else {
                for (let i = 0; i < count; i++)
                    sampleSizes.push(sample_size);
            }
        }
        else if (t === 'stco') {
            ...
            for (let i = 0; i < entry_count; i++) {
                chunkOffsets.push(dv.getUint32(p));
                p += 4;
            }
        }
        else if (t === 'co64') {
            ...
            for (let i = 0; i < entry_count; i++) {
                ...
                chunkOffsets.push(Number((BigInt(hi) << 32n) | BigInt(lo)));
                p += 8;
            }
        }
        else if (t === 'stsc') {
            ...
            let p = ptr + 16;
            for (let i = 0; i < entry_count; i++) {
                const first = dv.getUint32(p);
                const samples = dv.getUint32(p + 4); /*const desc = dv.getUint32(p+8)*/
                stsc.push({ first, samples });
                p += 12;
            }
        }
        else if (t === 'stts') {
            ...
            for (let i = 0; i < entry_count; i++) {
                ...
                const d = dv.getUint32(p + 4);
                stts.push({ count: c, delta: d });
                p += 8;
            }
        }
        else if (t === 'stss') {
            ...
            let p = ptr + 16;
            for (let i = 0; i < entry_count; i++) {
                syncSamples.add(dv.getUint32(p));
                p += 4;
            }
        }
        ptr += sz;
    }
    if (!sampleCount || !sampleSizes.length || !chunkOffsets.length)
        return [];
    // expand stsc into per-chunk sample counts
    const chunkSampleCounts = [];
    for (let i = 0; i < chunkOffsets.length; i++)
        chunkSampleCounts.push(0);
    for (let i = 0; i < stsc.length; i++) {
        ...
        for (let c = start; c <= end; c++)
            chunkSampleCounts[c] = stsc[i].samples;
    }
        ...
        for (let s = 0; s < samplesInChunk; s++) {
            ...
            if (sEnd <= len) {
                ...
                for (const e of stts) {
                    ...
                    pts += BigInt(take) * BigInt(e.delta);
                    remaining -= take;
                    if (remaining <= 0)
                        break;
                }
...
function extractSpsPpsFromAvcC(avcC) {
    const out = { sps: [], pps: [], nalLengthSize: 4 };
    ...
    try {
        ...
        for (let i = 0; i < spsCount; i++) {
            const sl = (avcC[off] << 8) | avcC[off + 1];
            off += 2;
        ...
        for (let i = 0; i < ppsCount; i++) {
            const pl = (avcC[off] << 8) | avcC[off + 1];
            off += 2;
...
function avccToAnnexB(buf, nalLengthSize = 4) {
    ...
    while (o + nalLengthSize <= u8.length) {
        ...
        for (let i = 0; i < nalLengthSize; i++) {
            nalLen = (nalLen << 8) | u8[o + i];
        }
        o += nalLengthSize;
        if (nalLen <= 0 || o + nalLen > u8.length)
            break;
...
export { avccToAnnexB, extractSpsPpsFromAvcC, parseFragment, parseInit, parseMoovSamples };
//# sourceMappingURL=demux-mp4-2b5612b7.js.map
